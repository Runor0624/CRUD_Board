...
function PostAddModal () {
    ...
    const onSubmit = useCallback((e:React.FormEvent) => {
        e.preventDefault()
        axios.post(`${axios.defaults.baseURL}/post`, {
            headers: {
...
<Container onSubmit={handleSubmit(() => onSubmit)}>
    <Label>제목</Label>
    <Input {...register('title', {required: '제목은 필수 입력입니다' , minLength: {value: 3, message:'3자 이상 입력하시오!'}})} type='text' onChange={onChangeTitle} tabIndex={1} />
    ...
    <Label>가격</Label>
    <Input {...register('price', {minLength: {value: 3, message:'3자 이상 입력하시오!'}})} type='number' onChange={onChnagePrice} tabIndex={4}/>

    <Label>할인가</Label>
    <Input {...register('Discount', {minLength: {value: 3, message:'3자 이상 입력하시오!'}})} type="text" onChange={onChangeDiscount} tabIndex={5} />
